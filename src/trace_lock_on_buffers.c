...
LockOnBuffersTraceEndWrite(StringInfo stream, bool resultIsEmpty)
{
    ...
    {
        stream->data[stream->len - 2] = ' '; //delete last ',' in array
        appendStringInfo(stream, "]\n");
    }
}
...
LockOnBuffersTraceWriteOneLock(StringInfo stream, BufferLWLockStatData* lwlockData)
{
    ...
    if (relForTag == NULL)
        return false;
    namespaceName = get_namespace_name_or_temp(relForTag->rd_rel->relnamespace); //namespace name
#if PG_MAJORVERSION_NUM > 15
        dbName = get_database_name(lwlockData->bufferTag.dbOid); //db name
        spcName = get_tablespace_name(lwlockData->bufferTag.spcOid); //tablespace name
#else
        dbName = get_database_name(lwlockData->bufferTag.rnode.dbNode); //db name
        spcName = get_tablespace_name(lwlockData->bufferTag.rnode.spcNode); //tablespace name
#endif
    if (writeMode == JSON_WRITE_MODE)
    {
        appendStringInfo(stream,
...
void
LockOnBuffersTraceClearStat(void)
{
    currentTraceLock = NULL;

    LIST_FOREACH(bufferLockStatStorageList, it)
    {
        hash_destroy((HTAB*) it->value);
    }
    ListMakeEmpty(bufferLockStatStorageList);
    ListAdd(bufferLockStatStorageList, BufferLWLockStatStorageInit());
}


void
LockOnBuffersTraceDumpCurrentStatWithPrefix(const char* info, const char* shortName)
{
    MemoryContext old;
    StringInfoData str;
    bool hasLWLockStat;
    ...
    if (hasLWLockStat)
    {
        if (writeMode == TEXT_WRITE_MODE)
            TracePrintf("TRACE LWLOCK. %s: %s", info, str.data);
        else
            TracePrintf(
                "\"%s\": {\n"
                "    \"info\": \"%s\",\n"
